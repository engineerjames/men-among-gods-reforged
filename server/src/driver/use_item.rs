...
pub fn sub_door_driver(gs: &mut GameState, _cn: usize, item_idx: usize) -> bool {
    ...
    if item.data[0] == 65501 || item.data[0] == 65502 {
        ...
        return (item.data[0] == 65501 && empty == 3 && star == 1)
            || (item.data[0] == 65502 && empty == 3 && circle == 1);
    }
...
pub fn use_door(gs: &mut GameState, cn: usize, item_idx: usize) -> bool {
    ...
    let locked_without_key = {
        ...
        if lock_code != 0 {
            ...
            gs.items[item_idx].data[1] != 0 && !lock
        } else {
            false
        }
    };
...
pub fn finish_laby_teleport(gs: &mut GameState, cn: usize, nr: usize, exp: usize) -> bool {
    ...
    for n in 0..40 {
        ...
        if item_idx != 0 && (gs.items[item_idx].flags & ItemFlags::IF_LABYDESTROY.bits()) != 0 {
...
pub fn teleport(gs: &mut GameState, cn: usize, item_idx: usize) -> bool {
    ...
    for n in 0..20 {
        let spell_idx = gs.characters[cn].spell[n] as usize;
        if spell_idx != 0 && gs.items[spell_idx].temp == skills::SK_RECALL as u16 {
            gs.characters[cn].spell[n] = 0;
            gs.items[spell_idx].used = USE_EMPTY;
        }
    }
...
pub fn use_crystal_sub(gs: &mut GameState, _cn: usize, item_idx: usize) -> i32 {
    ...
    for (i, &val) in baseg.iter().enumerate() {
        if val < tmpmin {
            tmpmin = val;
            sbase = i;
        }
    ...
    let m = loop {
        ...
        if player::commands::plr_check_target(gs, m_try) {
            break m_try;
        }
    };
    ...
    if helpers::random_mod_i32(30) == 0
        && gs.characters[cc].data[0] > 5
        && let Some(it) = God::create_item(gs, helpers::random_mod_usize(2) + 273)
    {
        God::give_character_item(gs, cc, it);
    }
    if helpers::random_mod_i32(60) == 0
        && gs.characters[cc].data[0] > 15
        && let Some(it) = God::create_item(gs, helpers::random_mod_usize(2) + 192)
    {
        God::give_character_item(gs, cc, it);
    }
    if helpers::random_mod_i32(150) == 0
        && gs.characters[cc].data[0] > 20
        && let Some(it) = God::create_item(gs, helpers::random_mod_usize(9) + 181)
    {
        God::give_character_item(gs, cc, it);
    }
...
pub fn use_grave(gs: &mut GameState, _cn: usize, item_idx: usize) -> bool {
    ...
    if cc > 0 {
        ...
        if is_alive {
            return true; // Still alive, don't spawn new one
        }
    }
...
pub fn solved_pentagram(gs: &mut GameState, cn: usize, item_idx: usize) -> bool {
    ...
    for n in 1..gs.items.len() {
        ...
        if gs.items[n].active == 0
            && gs.items[n].light[0] != gs.items[n].light[1]
            && gs.items[n].x > 0
        {
            gs.do_add_light(
            ...
            for m in 1..4 {
                let needs_spawn = {
                    ...
                    if co == 0 {
                        ...
                        co >= gs.characters.len()
                            || gs.characters[co].used == USE_EMPTY
                            || gs.characters[co].data[0] != item_idx as i32
...
pub fn spell_scroll(gs: &mut GameState, cn: usize, item_idx: usize) -> bool {
    ...
    if spell as usize == skills::SK_CURSE || spell as usize == skills::SK_STUN {
        ...
        if !driver::player_or_ghost(&gs.characters[cn], cn, &gs.characters[co]) {
...
pub fn use_driver(gs: &mut GameState, cn: usize, item_idx: usize, carried: bool) {
    ...
    if has_usespecial {
        ...
        let ret = match driver {
            ...
            36 => false, // magic portal 1 in lab13
            37 => false, // traps
            38 => false, // magic portal 2 in lab13
            39 => purple_protect(gs, cn, item_idx),
            40 => false, // seyan'du sword
            ...
            65 => crate::lab9::lab9_use_switch(gs, cn, item_idx as i32),
            66 => crate::lab9::lab9_use_door(gs, cn, item_idx as i32),
...
fn soul_trans_equipment(gs: &mut GameState, cn: usize, soulstone_idx: usize, item_idx: usize) {
    ...
    while rank > 0 {
        ...
        match ran {
            ...
            7..=9 => {
                ...
                item.skill[skills::SK_WEAPON][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            14 => {
                ...
                item.skill[skills::SK_STUN][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            15 => {
                ...
                item.skill[skills::SK_CURSE][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            16 => {
                ...
                item.skill[skills::SK_BLESS][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            17 => {
                ...
                item.skill[skills::SK_RESIST][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            18 => {
                ...
                item.skill[skills::SK_BLAST][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            19 => {
                ...
                item.skill[skills::SK_HEAL][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            20 => {
                ...
                item.skill[skills::SK_GHOST][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            21 => {
                ...
                item.skill[skills::SK_IMMUN][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            23 => {
                ...
                item.skill[skills::SK_CONCEN][2] = std::cmp::min(120, current + (stren * 5)) as i16;
            ...
            24 => {
                ...
                item.skill[skills::SK_WARCRY][2] = std::cmp::min(120, current + (stren * 5)) as i16;
    ...
    gs.items[item_idx].min_rank = std::cmp::max(soulstone_rank as i8, gs.items[item_idx].min_rank);
...
pub fn item_damage_armor(gs: &mut GameState, cn: usize, damage: i32) {
    let dam = damage / 4 + 1;
    for n in 0..20 {
        if n != WN_RHAND && n != WN_LHAND && helpers::random_mod_i32(3) != 0 {
...
pub fn item_tick_expire(gs: &mut GameState) {
    ...
    for x in 0..SERVER_MAPX as usize {
        ...
        if in_idx != 0 {
            ...
            if (flags & ItemFlags::IF_REACTIVATE.bits()) != 0
                && active == 0
                && !ch_present
                && !to_ch_present
            {
            ...
            if (((flags & ItemFlags::IF_TAKE.bits()) == 0
                || (map_flags & u64::from(MF_NOEXPIRE)) != 0)
                && driver != 7)
                || driver == 37
