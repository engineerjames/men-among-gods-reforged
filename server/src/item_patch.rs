//! Background watcher that polls KeyDB for admin-issued item patches and
//! applies them to the in-memory item table between ticks.
//!
//! The API writes each patch as a bincode [`ItemPatch`] entry pushed to
//! [`core::item_store::ITEM_PATCH_QUEUE_KEY`] (via `RPUSH`). A `POST` to
//! the reload endpoint flushes the queue by writing
//! [`core::item_store::ITEM_PATCH_REQUEST_KEY`]. This watcher polls both
//! keys and forwards [`ItemPatchEvent`]s to the tick thread, which merges
//! only the static fields into `GameState.items`, preserving the dynamic
//! fields (position, damage state, current age/damage, sprite override).

use core::item_store::{self, ITEM_PATCH_QUEUE_KEY, ITEM_PATCH_REQUEST_KEY, ItemPatch};
use redis::Commands;
...
pub const DISABLE_ENV: &str = "MAG_ADMIN_ITEM_PATCH_DISABLED";
...
pub struct ItemPatchWatcher {
    rx: Receiver<ItemPatchEvent>,
    shutdown: Arc<AtomicBool>,
    handle: Option<JoinHandle<()>>,
}

impl ItemPatchWatcher {
    ...
    pub fn spawn() -> Option<Self> {
        if std::env::var(DISABLE_ENV)
            ...
            .unwrap_or(false)
        {
            log::info!("Item patch watcher disabled via {} env var", DISABLE_ENV);
            return None;
        }
        ...
        let handle = thread::Builder::new()
            .name("item-patch-watcher".into())
            .spawn(move || watcher_loop(tx, shutdown_thread))
            .ok()?;

        log::info!("Item patch watcher started");
        Some(Self {
            rx,
            shutdown,
            handle: Some(handle),
    ...
    pub fn try_recv(&self) -> Option<ItemPatchEvent> {
...
impl Drop for ItemPatchWatcher {
    fn drop(&mut self) {
...
fn watcher_loop(tx: Sender<ItemPatchEvent>, shutdown: Arc<AtomicBool>) {
    ...
    while !shutdown.load(Ordering::SeqCst) {
        ...
        if let Some(ref request_id) = reload_request_id {
            let status_key = item_store::item_patch_status_key(request_id);
            if let Err(e) = conn.set_ex::<_, _, ()>(&status_key, STATUS_PENDING, STATUS_TTL_SECS) {
                log::warn!("item patch watcher: status SET failed: {}", e);
            }
        }
        ...
        loop {
            let batch = match drain_batch(conn) {
                Ok(b) => b,
                Err(e) => {
                    log::warn!("item patch watcher: drain failed: {}", e);
                    con = None;
                    break;
                }
            };
            ...
            for patch in batch {
                if tx.send(ItemPatchEvent::Apply(patch)).is_err() {
...
fn drain_batch(conn: &mut redis::Connection) -> Result<Vec<ItemPatch>, String> {
    let raw: Vec<Vec<u8>> = redis::cmd("LPOP")
        .arg(ITEM_PATCH_QUEUE_KEY)
        .arg(DRAIN_BATCH)
        .query(conn)
        .map_err(|e| format!("LPOP {}: {}", ITEM_PATCH_QUEUE_KEY, e))?;

    let mut out = Vec::with_capacity(raw.len());
    for bytes in raw {
        match ItemPatch::from_bytes(&bytes) {
            Ok(p) => out.push(p),
            Err(e) => log::warn!("item patch watcher: dropping undecodable patch: {}", e),
        }
    }
    Ok(out)
}
...
pub fn write_applied_status(con: &mut redis::Connection, request_id: &str) -> Result<(), String> {
    let key = item_store::item_patch_status_key(request_id);
