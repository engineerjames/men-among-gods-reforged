//! Background watcher that polls KeyDB for admin-issued map-tile patches
//! and applies them to the in-memory map between ticks.
//!
//! The API writes each patch as a bincode [`MapPatch`] entry pushed to
//! [`core::map_store::MAP_PATCH_QUEUE_KEY`] (via `RPUSH`). A `POST` to the
//! reload endpoint flushes the queue by writing
//! [`core::map_store::MAP_PATCH_REQUEST_KEY`]. This watcher polls both keys
//! and forwards [`MapPatchEvent`]s to the tick thread, which merges only
//! the static fields into `GameState.map`, preserving the dynamic fields
//! (`ch`, `to_ch`, `it`, `light`, `dlight`).

use core::map_store::{self, MAP_PATCH_QUEUE_KEY, MAP_PATCH_REQUEST_KEY, MapPatch};
use redis::Commands;
...
pub const DISABLE_ENV: &str = "MAG_ADMIN_MAP_PATCH_DISABLED";
...
pub enum MapPatchEvent {
    /// Apply a single patch to the in-memory map.
    Apply(MapPatch),
...
pub struct MapPatchWatcher {
    rx: Receiver<MapPatchEvent>,
    shutdown: Arc<AtomicBool>,
    handle: Option<JoinHandle<()>>,
}

impl MapPatchWatcher {
    ...
    pub fn spawn() -> Option<Self> {
        if std::env::var(DISABLE_ENV)
            ...
            .unwrap_or(false)
        {
            log::info!("Map patch watcher disabled via {} env var", DISABLE_ENV);
            return None;
        }
        ...
        let handle = thread::Builder::new()
            .name("map-patch-watcher".into())
            .spawn(move || watcher_loop(tx, shutdown_thread))
            .ok()?;

        log::info!("Map patch watcher started");
        Some(Self {
            rx,
            shutdown,
            handle: Some(handle),
    ...
    pub fn try_recv(&self) -> Option<MapPatchEvent> {
...
fn watcher_loop(tx: Sender<MapPatchEvent>, shutdown: Arc<AtomicBool>) {
    ...
    while !shutdown.load(Ordering::SeqCst) {
        ...
        if con.is_none() {
            match super::connection::connect() {
                Ok(c) => con = Some(c),
                Err(e) => {
                    log::warn!("map patch watcher: keydb connect failed: {}", e);
                    continue;
                }
            }
        }
        ...
        if let Some(ref request_id) = reload_request_id {
            let status_key = map_store::map_patch_status_key(request_id);
            if let Err(e) = conn.set_ex::<_, _, ()>(&status_key, STATUS_PENDING, STATUS_TTL_SECS) {
                log::warn!("map patch watcher: status SET failed: {}", e);
            }
        }

        // When a reload was explicitly requested we drain the queue fully;
        // otherwise we drain at most one batch per poll for snappy latency
        ...
        loop {
            let batch = match drain_batch(conn) {
                Ok(b) => b,
                Err(e) => {
                    log::warn!("map patch watcher: drain failed: {}", e);
                    con = None;
                    break;
                }
            };
            ...
            for patch in batch {
                if tx.send(MapPatchEvent::Apply(patch)).is_err() {
...
fn drain_batch(conn: &mut redis::Connection) -> Result<Vec<MapPatch>, String> {
    let raw: Vec<Vec<u8>> = redis::cmd("LPOP")
        .arg(MAP_PATCH_QUEUE_KEY)
        .arg(DRAIN_BATCH)
        .query(conn)
        .map_err(|e| format!("LPOP {}: {}", MAP_PATCH_QUEUE_KEY, e))?;

    let mut out = Vec::with_capacity(raw.len());
    for bytes in raw {
        match MapPatch::from_bytes(&bytes) {
            Ok(p) => out.push(p),
            Err(e) => log::warn!("map patch watcher: dropping undecodable patch: {}", e),
        }
    }
    Ok(out)
}
...
pub fn write_applied_status(con: &mut redis::Connection, request_id: &str) -> Result<(), String> {
    let key = map_store::map_patch_status_key(request_id);
