...
impl GameState {
    ...
    pub(crate) fn do_shop_char(&mut self, cn: usize, co: usize, nr: i32, autoloot: i32) {
        ...
        if is_body {
            ...
            let co_x = i32::from(self.characters[co].x);
            let co_y = i32::from(self.characters[co].y);
        ...
        if citem != 0 && is_merchant {
            ...
            if !accepts {
                let merchant_name = self.characters[co].get_name().to_owned();
            ...
            if merchant_gold < price {
                let merchant_ref = self.characters[co].get_reference().to_owned();
                self.do_character_log(
                    cn,
                    FontColor::Green,
            ...
            if nr < 62 {
                ...
                if item_idx != 0 {
                    ...
                    if gave_success {
                        ...
                        let item_ref =
                            c_string_to_str(&mut self.items[item_idx as usize].reference).to_owned();
