...
void do_area_say1( int cn, int xs, int ys, char* msg )
{
  ...
  for ( j = 0; j < SPIRALSIZE; j++ )
  {
    ...
    if ( ! IS_SANECHAR( cc ) )
      continue;
    if ( ( ch[ cc ].flags & ( CF_PLAYER | CF_USURP ) ) )
    { // listener is a player
    ...
    { // listener is NPC: Store in list for second pass
      // DB: note: this should be changed for staff/god NPCs
      if ( ! invis && cnt < ARRAYSIZE( npcs ) )
      { // NPCs pretend not to hear invis people
        if ( j < 169 )
        { // don't address mobs outside radius 6
          npcs[ cnt++ ] = cc;
        }
      }
    }
...
void do_tell( int cn, char* con, char* text )
{
  ...
  if ( ! ( ch[ co ].flags & ( CF_PLAYER ) ) || ch[ co ].used != USE_ACTIVE ||
       ( ( ch[ co ].flags & CF_INVISIBLE ) && invis_level( cn ) < invis_level( co ) ) ||
...
void do_shout( int cn, char* text )
{
  ...
  for ( n = 1; n < MAXCHARS; n++ )
  {
    if ( ( ( ch[ n ].flags & ( CF_PLAYER | CF_USURP ) ) || ch[ n ].temp == 15 ) && ch[ n ].used == USE_ACTIVE &&
         ( ( ! ( ch[ n ].flags & CF_NOSHOUT ) && ! do_is_ignore( cn, n, 0 ) ) || ( ch[ cn ].flags & CF_GOD ) ) )
    {
...
void do_group( int cn, char* name )
{
  ...
  if ( name[ 0 ] == 0 )
  {
    ...
    do_char_log( cn, 1, "%-15.15s %d/%dH, %d/%dE, %d/%dM\n", ch[ cn ].name, ( ch[ cn ].a_hp + 500 ) / 1000, ch[ cn ].hp[ 5 ],
                 ( ch[ cn ].a_end + 500 ) / 1000, ch[ cn ].end[ 5 ], ( ch[ cn ].a_mana + 500 ) / 1000, ch[ cn ].mana[ 5 ] );
    for ( n = 1; n < 10; n++ )
    {
      ...
      if ( isgroup( co, cn ) )
      {
        do_char_log( cn, 1, "%-15.15s %d/%dH, %d/%dE, %d/%dM\n", ch[ co ].name, ( ch[ co ].a_hp + 500 ) / 1000, ch[ co ].hp[ 5 ],
                     ( ch[ co ].a_end + 500 ) / 1000, ch[ co ].end[ 5 ], ( ch[ co ].a_mana + 500 ) / 1000, ch[ co ].mana[ 5 ] );
      }
      else
      {
...
void do_follow( int cn, char* name )
{
  ...
  if ( ch[ co ].flags & ( CF_INVISIBLE | CF_NOWHO ) && invis_level( co ) > invis_level( cn ) )
  {
    do_char_log( cn, 0, "Sorry, I cannot find %s.\n", name );
    return;
  }
...
void do_emote( int cn, char* text )
{
  ...
  else if ( ch[ cn ].flags & CF_INVISIBLE )
  { // JC: 091200: added anonymous emote
...
void do_create_note( int cn, char* text )
{
  ...
  for ( m = 0; m < 40; m++ )
  { // looking for free space in inventory
    if ( ch[ cn ].item[ m ] == 0 )
    {
...
void do_stat( int cn )
{
  ...
  do_char_log( cn, 2, "mayhem=%s, looting=%s, close=%s, cap=%s, speedy=%s\n", globs->flags & GF_MAYHEM ? "yes" : "no",
               globs->flags & GF_LOOTING ? "yes" : "no", globs->flags & GF_CLOSEENEMY ? "yes" : "no", globs->flags & GF_CAP ? "yes" : "no",
...
void do_list_net( int cn, int co )
{
  ...
  for ( n = 80; n < 90; n++ )
    do_char_log( cn, 1, "%d.%d.%d.%d\n", ch[ co ].data[ n ] & 255, ( ch[ co ].data[ n ] >> 8 ) & 255, ( ch[ co ].data[ n ] >> 16 ) & 255,
                 ( ch[ co ].data[ n ] >> 24 ) & 255 );
}
...
void do_char_killed( int cn, int co )
{
  ...
  else if ( ch[ co ].temp >= 375 && ch[ co ].temp <= 381 )
  { // hack for gargoyles
  ...
  if ( cn && cn != co && ( ch[ cn ].flags & ( CF_PLAYER ) ) && ! ( mf & MF_ARENA ) )
  {
    ...
    // becoming purple
    if ( ch[ co ].temp == CT_PRIEST )
    { // add all other priests of the purple one here...
      if ( ch[ cn ].kindred & KIN_PURPLE )
      {
    ...
    {
      if ( ch[ co ].monsterClass && ! killed_class( cn, ch[ co ].monsterClass ) )
      {
        do_char_log( cn, 0, "You just killed your first %s. Good job.\n", get_class_name( ch[ co ].monsterClass ) );
  ...
  if ( cn && cn != co && ! ( ch[ cn ].flags & ( CF_PLAYER ) ) && ( cc = ch[ cn ].data[ 63 ] ) != 0 && ( ch[ cc ].flags & ( CF_PLAYER ) ) )
  {
    ...
    {
      do_char_log( cc, 0,
                   "A goddess is about to turn your follower into a frog, but notices that you are responsible. You feel her do something "
                   "to you. Nothing good, that's for sure.\n" );
...
int do_char_can_flee( int cn )
{
  ...
  if ( ! ch[ cn ].enemy[ 0 ] && ! ch[ cn ].enemy[ 1 ] && ! ch[ cn ].enemy[ 2 ] && ! ch[ cn ].enemy[ 3 ] )
    return 1;
...
{
  if ( ch[ cn ].enemy[ 0 ] != co && ch[ cn ].enemy[ 1 ] != co && ch[ cn ].enemy[ 2 ] != co && ch[ cn ].enemy[ 3 ] != co )
  {
    if ( ! ch[ cn ].enemy[ 0 ] )
      ch[ cn ].enemy[ 0 ] = co;
...
void do_give_exp( int cn, int p, int gflag, int rank )
{
  ...
  if ( gflag )
  {
    ...
    { // we're an NPC
      ...
      { // we are the follower of someone
...
int do_hurt( int cn, int co, int dam, int type )
{
  ...
  if ( ch[ co ].a_hp - dam < 500 && ch[ co ].luck >= 100 && ! ( mf & MF_ARENA ) && RANDOM( 10000 ) < 5000 + ch[ co ].luck )
  {
    ...
    ch[ co ].luck /= 2;
    do_char_log( co, 0,
                 "A god reached down and saved you from the killing blow. You must have done the gods a favor sometime in the past!\n" );
...
void do_regenerate( int cn )
{
  ...
  if ( ! ch[ cn ].stunned )
  {
    ...
    case 176:
    case 184:
      if ( ch[ cn ].status2 == 0 || ch[ cn ].status2 == 5 || ch[ cn ].status2 == 6 )
      { // attack
        if ( ch[ cn ].mode == 1 )
        {
          ch[ cn ].a_end -= 12;
  ...
  if ( ( in = ch[ cn ].worn[ WN_NECK ] ) && it[ in ].temp == 768 )
  { // amulet of ankh
    if ( ch[ cn ].skill[ SK_REGEN ][ 0 ] )
      ...
      else
      {
        ...
        if ( it[ in ].active == TICKS * 30 )
        {
          ...
          else
          {
            ...
            if ( ch[ cn ].temp == CT_COMPANION && ( co = ch[ cn ].data[ 63 ] ) && IS_SANEPLAYER( co ) &&
                 ( it[ in ].temp == SK_BLESS || it[ in ].temp == SK_PROTECT || it[ in ].temp == SK_ENHANCE ) )
            {
              do_sayx( cn, "My spell %s is running out, %s.", it[ in ].name, ch[ co ].name );
            }
          }
        }
      }
...
void do_look_item( int cn, int in )
{
  ...
  else
  {
    ...
    if ( IS_BUILDING( cn ) )
    {
      ...
      do_char_log( cn, 1, "Driver=%d [%d,%d,%d,%d,%d,%d,%d,%d,%d,%d].\n", it[ in ].driver, it[ in ].data[ 0 ], it[ in ].data[ 1 ],
                   it[ in ].data[ 2 ], it[ in ].data[ 3 ], it[ in ].data[ 4 ], it[ in ].data[ 5 ], it[ in ].data[ 6 ], it[ in ].data[ 7 ],
                   it[ in ].data[ 8 ], it[ in ].data[ 9 ] );
    }
    ...
    if ( ch[ cn ].flags & CF_GOD )
    {
      ...
      do_char_log( cn, 2, "max_age=%d/%d, current_age=%d/%d\n", it[ in ].max_age[ 0 ], it[ in ].max_age[ 1 ], it[ in ].current_age[ 0 ],
                   it[ in ].current_age[ 1 ] );
...
void do_pay_depot( int cn )
{
  ...
  while ( 1 )
  {
    ...
    if ( c > ch[ cn ].data[ 13 ] )
    {
      ...
      ch[ cn ].depot[ ln ] = 0;
      ch[ cn ].depot_sold++;
      chlog( cn, "Bank sold %s for %dG %dS to pay for depot (slot %d)", it[ in ].name, ( lv / 2 ) / 100, ( lv / 2 ) % 100, ln );
    }
    else
    {
      ch[ cn ].data[ 13 ] -= c;
...
void do_look_char( int cn, int co, int godflag, int autoflag, int lootflag )
{
  ...
  if ( ! autoflag && ! ( ch[ co ].flags & CF_MERCHANT ) && ! ( ch[ co ].flags & CF_BODY ) )
  {
    ...
    if ( ! godflag && cn != co && ( ch[ cn ].flags & ( CF_PLAYER ) ) && ! ( ch[ cn ].flags & CF_INVISIBLE ) &&
  ...
  *( unsigned short* ) ( buf + 13 ) = ( ch[ co ].a_mana + 500 ) / 1000 + mana_diff;
  ...
  if ( ( ch[ cn ].flags & ( CF_GOD | CF_IMP | CF_USURP ) ) && ! autoflag && ! ( ch[ co ].flags & CF_MERCHANT ) &&
       ! ( ch[ co ].flags & CF_BODY ) && ! ( ch[ co ].flags & CF_GOD ) )
  {
...
{
  int          tmp, in, m;
  static char* at_text[ 5 ] = { "not brave enough", "not determined enough", "not intuitive enough", "not agile enough",
                                "not strong enough" };
  ...
  if ( tmp )
  {
    if ( it[ tmp ].driver == 52 && it[ tmp ].data[ 0 ] != cn )
    {
      if ( it[ tmp ].data[ 0 ] == 0 )
      {
        ...
        sprintf( buf, "%s Engraved in it are the letters \"%s\".", it[ tmp ].description, ch[ cn ].name );
        if ( strlen( buf ) < 200 )
...
void do_seen( int cn, char* cco )
{
  ...
  if ( ch[ cn ].flags & ( CF_IMP | CF_GOD ) )
  {
    ...
    do_char_log( cn, 2, "%s was last seen on %04d-%02d-%02d %02d:%02d:%02d (time now: %04d-%02d-%02d %02d:%02d:%02d)\n", ch[ co ].name,
                 tlast.tm_year + 1900, tlast.tm_mon + 1, tlast.tm_mday, tlast.tm_hour, tlast.tm_min, tlast.tm_sec, tnow.tm_year + 1900,
