...
int add_spell( int cn, int in )
{
  ...
  if ( n == 20 )
  {
    ...
    if ( n == 20 )
    { // overwrite weakest spell if it is weaker than the new spell
...
{
  return ch[ cn ].attrib[ AT_AGIL ][ 0 ] + ch[ cn ].attrib[ AT_STREN ][ 0 ] + ch[ cn ].attrib[ AT_INT ][ 0 ] +
         ch[ cn ].attrib[ AT_WILL ][ 0 ] + ch[ cn ].attrib[ AT_BRAVE ][ 0 ];
}
...
char* at_name[ 5 ] = { "Braveness", "Willpower", "Intuition", "Agility", "Strength" };
...
void item_info( int cn, int in, int look )
{
  ...
  if ( it[ in ].power )
    do_char_log( cn, 1, "%-12.12s %+4d", "Power", it[ in ].power );

  if ( it[ in ].min_rank )
    do_char_log( cn, 1, "%-12.12s %+4d", "Min. Rank", it[ in ].min_rank );
}
...
void skill_ghost( int cn )
{
  ...
  if ( ch[ cc ].attrib[ AT_AGIL ][ 0 ] >= 90 && ch[ cc ].attrib[ AT_STREN ][ 0 ] >= 90 )
  { // titanium
    ch[ cc ].armor_bonus  = 48 + 32;
    ch[ cc ].weapon_bonus = 40 + 32;
  }
  else if ( ch[ cc ].attrib[ AT_AGIL ][ 0 ] >= 72 && ch[ cc ].attrib[ AT_STREN ][ 0 ] >= 72 )
  { // crystal
    ch[ cc ].armor_bonus  = 36 + 28;
    ch[ cc ].weapon_bonus = 32 + 28;
  }
  else if ( ch[ cc ].attrib[ AT_AGIL ][ 0 ] >= 40 && ch[ cc ].attrib[ AT_STREN ][ 0 ] >= 40 )
  { // gold
    ch[ cc ].armor_bonus  = 30 + 24;
    ch[ cc ].weapon_bonus = 24 + 24;
  }
  else if ( ch[ cc ].attrib[ AT_AGIL ][ 0 ] >= 24 && ch[ cc ].attrib[ AT_STREN ][ 0 ] >= 24 )
  { // steel
    ch[ cc ].armor_bonus  = 24 + 20;
    ch[ cc ].weapon_bonus = 16 + 20;
  }
  else if ( ch[ cc ].attrib[ AT_AGIL ][ 0 ] >= 16 && ch[ cc ].attrib[ AT_STREN ][ 0 ] >= 16 )
  { // bronze
    ch[ cc ].armor_bonus  = 18 + 16;
    ch[ cc ].weapon_bonus = 8 + 16;
  }
  else if ( ch[ cc ].attrib[ AT_AGIL ][ 0 ] >= 12 && ch[ cc ].attrib[ AT_STREN ][ 0 ] >= 12 )
  { // leather
    ch[ cc ].armor_bonus  = 12 + 12;
    ch[ cc ].weapon_bonus = 8 + 12;
  }
  else if ( ch[ cc ].attrib[ AT_AGIL ][ 0 ] >= 10 && ch[ cc ].attrib[ AT_STREN ][ 0 ] >= 10 )
  { // cloth
  ...
  else if ( points2rank( ch[ cc ].points_tot ) < 6 )
  { // GC not yet Master Sergeant
    do_sayx( cc,
             "I shall defend you and obey your commands, %s. "
