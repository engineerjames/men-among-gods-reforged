//! Skills / character / talent-tree panel.
//!
//! Currently renders as a blank semi-transparent window. The panel content
//! will be filled in by a follow-up task.
...
/// The skills / character / talent-tree HUD panel.
///
...
impl Widget for SkillsPanel {
    ...
    fn render(&mut self, ctx: &mut RenderContext) -> Result<(), String> {
        ...
        font_cache::draw_text_centered(ctx.canvas, ctx.gfx, 1, "Skills", title_x, title_y)?;
...
mod tests {
    ...
    #[test]
    fn starts_hidden() {
        let panel = SkillsPanel::new(Bounds::new(0, 0, 100, 100), Color::RGBA(0, 0, 0, 180));
        assert!(!panel.is_visible());
    }

    #[test]
    fn toggle_flips_visibility() {
        let mut panel = SkillsPanel::new(Bounds::new(0, 0, 100, 100), Color::RGBA(0, 0, 0, 180));
        panel.toggle();
        assert!(panel.is_visible());
        panel.toggle();
        assert!(!panel.is_visible());
    ...
    fn hidden_panel_ignores_clicks() {
        let mut panel = SkillsPanel::new(Bounds::new(10, 10, 100, 100), Color::RGBA(0, 0, 0, 180));
